...
export default function VocabInputStep({
  ...
  return (
    <CardContainer className="max-w-2xl mx-auto">
      <h2 className="font-bold text-slate-900 text-lg mb-1">Soạn bài bằng AI ✨</h2>
      ...
      <label className="block text-sm font-semibold text-slate-900 mb-1.5">Danh sách từ vựng</label>
      <textarea
        value={vocabText}
        onChange={(e) => onVocabTextChange(e.target.value)}
        rows={8}
        placeholder={"Family - Gia đình\nFriend - Bạn bè\nSchool - Trường học\n..."}
        className="w-full rounded-2xl border border-surface-border bg-white p-4 text-sm text-slate-900 outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-200 resize-none font-mono"
