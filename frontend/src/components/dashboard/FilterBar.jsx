...
export default function FilterBar({
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 mb-6">
      ...
      <div className="flex-1">
        <label className="block text-xs font-semibold text-slate-900 mb-1">Buổi học</label>
        <select
          value={selectedSessionId ?? ""}
          onChange={(e) => onSessionChange(e.target.value)}
          disabled={sessions.length === 0}
          className="w-full h-11 rounded-2xl border border-surface-border bg-white px-4 text-sm font-semibold text-slate-900 outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-200 disabled:opacity-50"
        >
