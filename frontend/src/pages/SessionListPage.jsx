      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap mt-0.5">
            {s.className && (
              <span className="text-xs bg-surface-soft text-slate/70 px-2 py-0.5 rounded-full">
                {s.className}
              </span>
            )}
            {s.publishedAt && (
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {!locked && (
            <>
              <button
                className="text-slate/70 hover:text-pink-600 text-lg"
  ...
  return (
    <div className="min-h-screen bg-pink-50 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        {sessions === null && (
          <p className="text-sm text-slate/70">Đang tải danh sách buổi học...</p>
        )}

        {sessions?.length === 0 && (
          <p className="text-sm text-slate/70">Chưa có buổi học nào được giao.</p>
        )}

        {/* Bài tập của lớp mình */}
        {ownSessions.length > 0 && (
          <div className="space-y-3 mb-6">
            <p className="text-xs font-bold text-slate/70 uppercase tracking-wide">Lớp của bạn</p>
